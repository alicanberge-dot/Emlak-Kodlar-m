...
with tab1:
    ...
    with col_t:
        ...
        for i, p in enumerate(st.session_state.kayitlar):
            # BAŞLIK GÜNCELLEMESİ: Tür - Oda Sayısı - Konum
            baslik = f"🏠 {p.get('Tür')} - {p.get('Oda')} - {p.get('Konum')}"
            with st.expander(baslik):
                ...
                st.write(f"📞 {p.get('Telefon')} | 📝 {p.get('Not','')}")
...
with tab2:
    ...
    with col_tf:
        ...
        t_not = st.text_area("Müşteri Notları (Geri dönüş vb.):", key="t_not_in") # Geri dönüş tarihi yerine Notlar
        
        if st.button("Talebi Kaydet", use_container_width=True):
            yeni_t = {
                "Müşteri Adı": t_ad, "Telefon": t_tel, "Meslek": t_mes, "Tür": t_tur,
                "Oda": t_oda, "Bütçe Aralığı": float(t_butce), "Not": t_not,
...
# --- TAB 3: AKILLI EŞLEŞTİRME (RENKLİ ANALİZ) ---
with tab3:
    ...
    if st.session_state.kayitlar and st.session_state.talepler:
        for t in st.session_state.talepler:
            for p in st.session_state.kayitlar:
                ...
                elif p_fiyat <= t_butce * 1.15: skor += 10 # Esnek bütçe
                
                if skor >= 60:
                    st.success(f"💎 **UYUM ORANI: %{skor}**")
                    c1, c2, c3 = st.columns(3)
                    
                    # Renkli özellik gösterimi
                    c1.markdown(f"**Tür:** :{'green' if match_tur else 'red'}[{p.get('Tür')}]")
                    c2.markdown(f"**Oda:** :{'green' if match_oda else 'red'}[{p.get('Oda')}]")
                    c3.markdown(f"**Bütçe:** :{'green' if match_fiyat else 'red'}[{p_fiyat:,.0f} TL]")
                    
                    st.write(f"🤝 **Müşteri:** {t.get('Müşteri Adı')} ↔️ **Portföy:** {p.get('Mülk Sahibi')} ({p.get('Konum')})")
                    st.divider()
...
with tab4:
    ...
    if st.session_state.kayitlar:
        ...
        st.download_button("Excel/CSV Olarak İndir", data=csv, file_name="Portfoy_Listesi.csv")
